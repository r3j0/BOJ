...
LL interval_min(int start, int end, int idx, int left, int right, int* error) {
	push(start, end, idx);
	...
	int mid = (start + end) / 2;
	int e1 = 0;
	LL r1 = interval_min(start, mid, idx * 2, left, right, &e1);
	int e2 = 0;
	LL r2 = interval_min(mid + 1, end, idx * 2 + 1, left, right, &e2);
...
LL interval_sum(int start, int end, int idx, int left, int right) {
	...
	if (left <= start && end <= right) {
		//printf("[%d-%d] %lld\n", start, end, tree[idx].sums);
		return tree[idx].sums;
	}
...
int main(void) {
	...
	for (int i = 0; i < q; i++) {
		...
		if (mode == 1) {
			...
			int e;
			printf("%lld\n", interval_min(0, n - 1, 1, l, r, &e));
		}
		else if (mode == 4) {
			printf("%lld\n", interval_sum(0, n - 1, 1, l, r));
		}
	}
	return 0;
}